...
exports.filter = function(req, res) {
    ...
    Template.count(criteria, function(err, total) {
        Template.find(criteria, { '_id': 0 }).select('name demo_url description tags thumbs responsive free browsers software_versions high_resolution year').skip((page - 1) * perPage).limit(perPage).exec(function(err, templates) {
            if (err) {
                templates = [];
            }
